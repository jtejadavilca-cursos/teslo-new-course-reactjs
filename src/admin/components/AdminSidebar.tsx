...
export const AdminSidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
    ...
    const getInitialsUserName = (fullname?: string) => {
        if (!fullname) return "";

        const fullnameTokens = fullname.toUpperCase().split(" ");

        return fullnameTokens.length > 1
            ? `${fullnameTokens[0].charAt(0) + fullnameTokens[1].charAt(0)}`
            : fullnameTokens[0].charAt(0);
    };
                ...
                <div className="p-4 border-t border-gray-200">
                    <div className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                        <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold">
                            {getInitialsUserName(user?.fullName)}
                        </div>
